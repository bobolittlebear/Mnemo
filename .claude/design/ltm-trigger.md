...
| **Pipeline 分批处理** | 若未来 Pipeline 改为多批顺序 LLM 调用，单窗口总耗时仍 ≤ 批数 × 120s，但会因 L3 每 20 轮封顶而不膨胀。不变式仍成立 |
...
```
ACTIVE ──(msg_count ≥ 20)──► [L3 增量提取] ──► ACTIVE (继续)
  │
...
Phase 3: 触发器接入（部分完成）
  ├── messageCounter (L3)        ✅
  ├── sessionController.endSession (L1)   ⬜ 待实现（无语义识别，仅调 executeTerminalTrigger('explicit')）
  ├── SessionTimeoutScanner (L2)          ⬜ 待实现
  └── Session 销毁 hook 级联清理 + 续聊重置   ⬜ 待实现（见 §2.1 / O10）
