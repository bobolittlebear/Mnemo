...
### 4. 工具调用（规划中 · 已有设计文档 `tool-calling.md`，尚未启动开发）

让 AI 按用户意图调用工具**直接操作笔记**（写入 / 修改 / 删除），对标飞书文档 AI 写作能力。当前仅完成方案设计，开发未启动；后续可扩展更多 Tool（搜索、计算、绘图……）。
...
| 长期记忆遗忘机制（软删 + 复活）     | ✅ 已实现          |
| 笔记工具调用（AI 写入）         | 📋 方案设计（`tool-calling.md`，未启动开发）        |
| 任务状态管理                | 📋 规划中                                  |
...
- 📄 新增设计文档 `tool-calling.md`：工具调用让 AI 直接操作笔记的方案设计（尚未启动开发）
...
- [ ] Tool Calling（设计文档 `tool-calling.md`：AI 直接操作笔记，尚未启动开发）
