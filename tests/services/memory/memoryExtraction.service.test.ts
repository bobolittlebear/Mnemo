...
describe('ExtractionService', () => {
    ...
    it('E11a - parseFacts 解析正常 JSON', () => {
        const raw = JSON.stringify({
            facts: [{ content: '测试', confidence: 0.9 }],
        });
    ...
    it('E13 - LLM 返回带多余空白的 dirty content，经全链路清洗后正确输出', async () => {
        mockedCreateChat.mockResolvedValue(fixtures.llmDirtyContentResponse);
        const result = (await service.extractFacts(
            fixtures.mockMessages,
        )) as any;
