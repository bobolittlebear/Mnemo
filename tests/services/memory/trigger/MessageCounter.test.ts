...
const skipped = (reason: 'LOCK' | 'TERMINAL' | 'PROCESSING'): TriggerResult => ({
  status: 'SKIPPED',
  reason,
});
...
function createFakeRedis(): RedisClient {
  ...
  const expire = vi.fn<(key: string, seconds: number) => Promise<number>>(async () => 1);
...
function createCoordinator(fn: () => Promise<TriggerResult>): TriggerCoordinator {
...
describe('MessageCounter', () => {
  ...
  it('未达阈值不触发 triggerThreshold', async () => {
    ...
    const counter = new MessageCounter({ coordinator, redis, threshold: 5 });
  ...
  it('SKIPPED/PROCESSING 不重置，下条消息再次触发', async () => {
    const coordinator = createCoordinator(async () => skipped('PROCESSING'));
    const counter = new MessageCounter({ coordinator, redis, threshold: 5 });
